"""
Erzeugt die LaTeX-Kurztabellen fuer den LNP-Stabilitaetstest (Results, §LNP).

Ausgabe:  lnp_tables_short.tex  (in den Masterarbeit-LaTeX-Ordner)

Datenstand: HEK aus der neuen Analyse, HUVEC aus den alten Einzeldateien.
  Konfluenz : absolut (%) und normiert (% von t0)
  GFP       : Total Green Object Area, hintergrund-korrigiert (in 10^3 µm²/Image)
Hintergrund-Korrektur (wie in den Plots): pro Zellart wird je Zeitpunkt der
Mittelwert der beiden nicht-leuchtenden Kontrollen (empty + reference,
null-normiert) abgezogen, sodass die Kontrollen auf der Nulllinie liegen.
Fehler: Spalte "Std Dev Well" (Standardabweichung ueber die Wells einer
Bedingung). Alle vier Exporte dieses Versuchs fuehren diese Spalte, HEK und
HUVEC also dasselbe Fehlermass.
Die langen Zeitreihen-Tabellen im Appendix wurden entfernt.
"""
...
HERE = Path(__file__).resolve().parent   # masterthesis-analysis-code
BASE = HERE.parent                       # SoSe26/Masterarbeit
LATEX_DIR = BASE / "Masterarbeit LaTeX"  # dorthin gehen die erzeugten Tabellen
...
# (Original-Spaltenname, Anzeigename) — Namen exakt wie in der Plot-Legende.
COND = [
...
# Nicht-leuchtende Kontrollen je Zellart -> Hintergrund-Basislinie fuer GFP.
GFP_BASELINE = {
...
def gfp_corrected(cond):
    """GFP-Signal, null-normiert und hintergrund-korrigiert (wie im Plot)."""
...
def short_gfp_norm():
    ...
    L.append(r"\begin{table}[H]")
    L.append(r"\centering")
    L.append(r"\caption{Confluence-normalised GFP signal in the LNP stability test, "
             r"i.e.\ the background-corrected total green-object area divided by the "
             r"phase-object confluence at each time point and expressed per 100\,\% "
