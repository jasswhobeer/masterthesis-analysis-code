"""
Incucyte-Zeitverlaeufe plotten  (LNP-Stabilitaets-/Transfektionstest)
=====================================================================
Einfach ausfuehren -> Fenster geht auf. Links Dateien waehlen, rechts
Titel / Normierung / Bedingungsnamen einstellen, dann "plotten".
Pro Datei entsteht  <dateiname>_plot.png  und  .pdf  im selben Ordner.

Dateiformat: Incucyte-Export (Tab-getrennt, Komma-Dezimalzahlen,
Kopfzeile mit "Elapsed", danach je Bedingung eine Spalte plus die
zugehoerige "(Std Err Img)"-Spalte).
"""
...
# ─────────────────────────────────────────────────────────────
#  EINSTELLUNGEN (Startwerte; im Fenster ueberschreibbar)
# ─────────────────────────────────────────────────────────────
INPUT_DIR = HERE
FILE_PATTERN = "*.txt"

# Namens-Tabelle: LINKS Original-Name, RECHTS Anzeige-Name (Startwert im Fenster).
CONDITION_RENAME = {
...
# Normierung pro Datei:
#   "keine"    -> Rohdaten (Log-Achse automatisch, wenn sinnvoll)
#   "relativ"  -> auf t=0 normiert, in % vom Startwert (jede startet bei 100 %)
#   "null"     -> Startwert (t=0) abgezogen, jede Kurve startet bei 0
NORMALIZE = {
    # Alte kombinierte Dateien (HEK + HUVEC zusammen)
    "Confluence.txt":                    "relativ",
    "green fluorescent total area.txt":  "null",
    # Neue, pro Zellart getrennte Dateien
    ...
    # Neue Analyse (HEK + HUVEC wieder kombiniert in einer Datei)
    "Confluence new analysis.txt":               "relativ",
    "Green Image Mean new analysis.txt":         "null",
}
NORMALIZE_DEFAULT = "keine"

# Titel pro Datei (Startwert im Fenster).
TITLES = {
    # Alte kombinierte Dateien
    ...
    # Neue Analyse (kombiniert)
    "Green Image Mean new analysis.txt":       "LNP Stability Test – GFP Expression",
}
#   Fallback: "vessel" | "metric" | "filename" | "" | fester Text
TITLE_DEFAULT = "vessel"

# Datei in mehrere Teilplots aufsplitten (z. B. HEK und HUVEC getrennt).
#   Leer lassen ({}) = eine Datei -> ein Plot.
#   match  = Schluesselwoerter im Original-Namen (Bedingung kommt in den Teilplot,
#            wenn EINES davon vorkommt), label = Zusatz im Titel,
#            suffix = Zusatz im Dateinamen. Teilplots nutzen Fehlerbalken.
SPLIT_PLOTS = {
    "Confluence.txt": [
...
# Fehlerdarstellung je Normierung: "ci" (95 %-Band), "errorbar" oder "keine".
ERROR_STYLE = {
    "relativ": "ci",
    "null":    "errorbar",
    "keine":   "errorbar",
}
CI_MULT = 1.96          # Faktor fuers Konfidenzband (1.96 = ~95 %)
CI_ALPHA = 0.09         # Deckkraft des Konfidenzbands (kleiner = dezenter)
ERRORBAR_EVERY = 4      # Fehlerbalken nur an jedem n-ten Zeitpunkt

# Linie gestrichelt, wenn das Schluesselwort im Original-Namen steckt (klein).
# Standard: durchgezogen. Hier: Referenzen gestrichelt.
LINESTYLE_BY_KEYWORD = {
    "reference": (0, (6, 2)),
}

# Incucyte haengt an jede Bedingung eine Fehlerspalte an. Je nach Export/Analyse
# heisst sie unterschiedlich – hier alle bekannten Varianten auflisten.
ERR_SUFFIXES = [" (Std Err Img)", " (Std Dev Well)", " (Std Err Well)"]
ERR_SUFFIX = ERR_SUFFIXES[0]   # Rueckwaerts-Kompatibilitaet
...
def build_series(df, pairs, mode):
    """Liefert [(cond, y, err), ...] je nach Normierung."""
    raw = []
...
# ─────────────────────────────────────────────────────────────
#  PLOT PRO DATEI  -> gibt den PNG-Pfad zurueck (oder None)
# ─────────────────────────────────────────────────────────────
...
def plot_target(path, title=None, renames=None, mode=None):
    """Plottet eine Datei; splittet in Teilplots, falls in SPLIT_PLOTS definiert."""
    path = Path(path)
...
# ─────────────────────────────────────────────────────────────
#  DOPPELPLOT: links roh, rechts normiert (eine Zellsorte)
# ─────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────
#  GFP-PLOT (null-normiert) – eine oder beide Zellarten
# ─────────────────────────────────────────────────────────────
def plot_gfp_combined(paths, out_path, title=None, renames=None, order=None,
                      conditions=None, baseline=None, conf_path=None):
    """GFP-Plot (null-normiert, Fehlerbalken).
    `paths` = eine kombinierte Datei (HEK+HUVEC) oder mehrere getrennte Dateien.
    `conditions` (optional) filtert auf z. B. nur HEK oder nur HUVEC.
    `baseline` (optional, Original-Spaltenname) = zeitabhaengiger Hintergrund,
       der pro Zeitpunkt abgezogen wird (Drift-Korrektur, nicht nur t0). Damit
       liegt die nicht-leuchtende Kontrolle auf der Nulllinie statt im Negativen.
    `conf_path` (optional) = Konfluenz-Datei; ist sie gesetzt, wird das (bereits
       hintergrund-korrigierte) Signal je Zeitpunkt durch die Konfluenz geteilt und
       auf 100 % Konfluenz hochgerechnet (Signal / Konfluenz * 100). So wird die
       Pro-Zell-Expression trotz zeitlich veraenderlicher Zellzahl vergleichbar.
    Die Nulllinie bleibt als Bezug sichtbar."""
    ...
    # Reihenfolge festlegen (steuert Farben + Legende). Fallback: Datei-Reihenfolge.
    cond_to_file = {}
    ordered = []
    ...
    # Zeitabhaengige Hintergrund-Basislinie (null-normiert) fuer die Drift-Korrektur.
    # `baseline` = Spaltenname (fuer alle) ODER dict {Bedingung: Baseline-Spalte}
    # (z. B. pro Zellart die eigene Referenz).
    baseline_cache = {}

    def baseline_series(bcols):
        # bcols = ein Spaltenname oder mehrere (dann Mittelwert der null-normierten
        # Reihen, z. B. empty + reference gemeinsam als Hintergrund).
    ...
    # dezente Nulllinie als Bezug (Basislinie der nicht-leuchtenden Bedingungen)
    ...
    for (p, cond, err), col in zip(ordered, colors):
        ...
        if by is not None:
            y = y - by                  # zeitabhaengigen Hintergrund abziehen
        if conf_df is not None and cond in conf_df.columns:
            ...
            scale = np.divide(100.0, conf, out=np.zeros_like(conf),
                              where=conf > 1e-6)   # auf 100 % Konfluenz hochrechnen
            y = y * scale
            if e is not None:
                e = e * scale
        ls = linestyle_for(cond)
    ...
    # Achse ueber den vollen Wertebereich (inkl. negativer Basislinie), damit
    # die nicht-leuchtenden Bedingungen als Gruppe sichtbar bleiben.
    pad = 0.05 * (ymax - ymin)
...
# ─────────────────────────────────────────────────────────────
#  THESIS-FIGUREN: die drei eingebundenen Bilder aus neuen Daten
# ─────────────────────────────────────────────────────────────
# Reihenfolge im kombinierten GFP-Plot (steuert Farben + Legende; wie alt).
GFP_ORDER = [
...
# Zielordner der Thesis (dort liegen die eingebundenen PNGs).
BILDER_DIR = (HERE / ".." / ".." / "Masterarbeit LaTeX" / "Bilder").resolve()


def build_thesis_figures(copy_to_bilder=True, titled=False):
    """Baut die vier Thesis-Figuren aus der neuen Analyse (HEK+HUVEC kombiniert).
    `titled=False` unterdrueckt die plot-internen Titel; in der Thesis liefern die
    (a)/(b)-Sub-Bildunterschriften die Zellart und eine gemeinsame Ueberschrift."""
    import shutil

    # HEK aus der neuen Analyse, HUVEC aus den alten Einzeldateien.
    ...
    # 1 + 2: Confluence-Doppelplots pro Zellart (Absolute | Normalised)
    ...
    # 3 + 4: GFP-Plots getrennt nach Zellart (Total Green Object Area,
    #        hintergrund-korrigiert mit empty + reference).
    ...
    #        (auf 100 % Konfluenz hochgerechnet -> Pro-Zell-Vergleich).
...
def run_gui():
    ...
    keys = []                 # Reihenfolge der Dateien (Schluessel)
    info = {}                 # key -> dict(path, conditions, title, mode, renames)
    current = {"key": None}
    cond_entries = []
    ...
    def add_paths(paths):
        ...
        for p in paths:
            ...
            key = str(p.resolve())
            if key in info:
                continue                 # Duplikate raus (auch / vs \)
            conds = valid_conditions(p)
            if not conds:
                continue                 # nur gueltige Incucyte-Dateien
            meta = read_meta(p)
            metric = meta.get("Metric", "Value")
...
# ─────────────────────────────────────────────────────────────
def main():
    # Thesis-Figuren (die drei eingebundenen Bilder) aus den neuen Daten bauen.
    if "--thesis" in sys.argv[1:]:
        print("Baue Thesis-Figuren aus den getrennten Datensaetzen …")
        build_thesis_figures()
        return
    # Nur ECHTE .txt-Dateien als Argumente (schuetzt vor Spyder/IPython "-f ...").
