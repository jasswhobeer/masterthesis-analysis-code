...
def stack(col):
    M = np.vstack([pl[col].to_numpy(float) for pl in plates]) / 1e3
...
ax.fill_between(t, m - s, m + s, color="black", alpha=0.06, zorder=1)
m, s = stack(NODYE)
ax.plot(t, m, color="grey", lw=1.6, ls=":", label="No-dye reference", zorder=2)
ax.fill_between(t, m - s, m + s, color="grey", alpha=0.12, zorder=1)
