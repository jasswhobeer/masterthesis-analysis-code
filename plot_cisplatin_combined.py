"""
Cisplatin Assay Wellplate – Plates 1-3 combined.

Normalisierung: Pro Platte und Kondition wird durch den Startwert (t=0) geteilt,
sodass alle Kurven bei 1.0 starten und Steigungen direkt vergleichbar sind.

Fehler: sigma_total = sqrt(sigma_bio^2 + sigma_tech^2)
  sigma_bio  = SD ueber die 3 Platten (normierte Werte)
  sigma_tech = Mittelwert der "Std Err Img" pro Zeitpunkt, ebenfalls normiert
"""
...
# Gemeinsame Zeitachse (alle Platten haben 0..84 h in 4-h-Schritten)
elapsed = plates[0]["Elapsed"].to_numpy(dtype=float)

# Pro Kondition: matrix [n_plates x n_time] der normierten Mittelwerte und der techn. SE
mean_norm = {}   # [3 x T]
tech_norm = {}   # [3 x T]
...
for cond in CONDITIONS:
    ...
    # Normierung pro Platte durch eigenen Startwert
    start = mat_mean[:, [0]]
    mean_norm[cond] = mat_mean / start
    tech_norm[cond] = mat_se / start  # gleiche Skalierung wie Mittelwert

# Plot ---------------------------------------------------------------------
# --- Einheitlicher Plot-Stil (siehe alle Thesis-Plots) ---
#   Ueberschrift 18 | Achsenbeschriftung 14 | Legende 14 (== Labels) | Ticks 12
...
for cond, label, color in zip(CONDITIONS, LABELS, colors):
    m = mean_norm[cond]            # [3 x T]
    t = tech_norm[cond]            # [3 x T]
