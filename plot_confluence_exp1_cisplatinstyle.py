"""
Fibrinogen Experiment 1 — confluence time course.
Re-styled to match the Cisplatin plots (viridis, closed box, unified font
sizes). The spread is drawn as a shaded +/- 1 SD band, not as error bars.

Data cleaning (Variante A — no MAD outlier filter):
  - non-positive readings (<= 0) are treated as invalid (device dropouts).
  - Well A1 (dried) at t = 60 h and 72 h shows focus-related invalid
    readings (~11 %) and is excluded at those two timepoints.
  Otherwise all n = 3 wells per condition are kept; the errorbar is the
  SD across the available wells per timepoint.
"""
...
# Wells are addressed by their plate position, not by a fixed column index. The
# column order of an IncuCyte export is not guaranteed, and a shifted export
# would silently reassign whole conditions without raising anything.
GROUPS = {
...
# Well A1 (dried) carries focus-related invalid readings at these timepoints.
FOCUS_ARTEFACT = {"A1": [60.0, 72.0]}
T_TOL = 0.5                         # h, tolerance when matching a timepoint
...
def column_of(well):
    """Index of the mean column of a well. Error columns end in their own
    suffix and therefore do not match."""
...
t = np.array(t)
M = np.array(rows)                  # rows = timepoints, cols = wells in COLUMNS order

# ── data cleaning (Variante A) ────────────────────────────────────────────
M[M <= 0] = np.nan                  # invalid device dropouts
for _well, _hours in FOCUS_ARTEFACT.items():
    for _h in _hours:
        M[np.abs(t - _h) <= T_TOL, WELL_INDEX[_well]] = np.nan

# ── Cisplatin plot style ──────────────────────────────────────────────────
