"""
Fibrinogen Coating Analysis
==============================
Kernänderungen gegenüber v2:
  - Fokus-Filter: nur Zellebene identifizieren (höchster Modus ≥ 3000 µm),
    KEIN Mindestanteil mehr als Gate. Mindestanforderungen:
    ≥ 8 valide Zeitpunkte UND Zeitspanne ≥ 40 h.
  - Fit ausschließlich auf fokus-validen Punkten → kein Einfluss von
    Fokus-Ausreißern auf die Kurve.
  - Fit-Kurve wird auf t = 0 … t_max extrapoliert (4 Modelle: Gompertz,
    Gompertz+Decline, 4PL, Linear; Wahl per AICc).
  - Mean-Linie im Plot = Mittelwert der Fit-Kurven aller passing Wells
    ausgewertet auf einem feinen Gitter → startet bei t = 0, glatt.
  - Individuelle Spaghetti = valide Rohdaten-Punkte (scatter), damit
    man den echten Datenfundus sieht.
  - Rho, Sawtooth: nur informative Metriken im Excel, kein Filter.

Input:  Fibrinogen_Analyse_Plate_[1|2]_[Confluence|Focus_Position].txt
Output: fibrinogen_v3_growth.png
        fibrinogen_v3_qc.png
        fibrinogen_v3_params.png
        fibrinogen_v3_passrate.png
        fibrinogen_v3_results.xlsx
"""
...
#  KONFIGURATION
# ─────────────────────────────────────────────────────────────
# the raw IncuCyte exports are not part of this repository,
# place them next to this script or adjust this path
...
# Fokus-Parameter
KDE_BW          = 50.0   # µm, KDE-Bandbreite
FOCUS_TOL       = 40.0   # µm, Toleranzband um Zellebenen-Modus
CELL_FOCUS_MIN  = 3000.0 # µm, Mindesthöhe für Zellebene

# Inklusions-Kriterien (minimal, nur strukturell)
MIN_VALID_PTS   = 8      # Mindest-Anzahl valider Zeitpunkte für Fit
MIN_TIME_SPAN   = 40.0   # h, Mindest-Zeitspanne der validen Punkte

# Plot-Gitter
T_GRID_N        = 500    # Anzahl Punkte für Fit-Gitter
T_REF           = 140.0  # h, Bezugszeit fuer das berichtete Plateau der
                         # linearen Fits (siehe fit_well). Entspricht dem
                         # Ende des Plot-Gitters.

# Explizit ausgeschlossene Wells. Die Begruendung ist datenbasiert und wird
# hier vollstaendig dokumentiert, damit der Ausschluss nachvollziehbar und
# ueberpruefbar bleibt. Es handelt sich NICHT um eine Praeregistrierung.
# P1 Dried A5: 26/28 valide Punkte bei 0.0%, 2 Fokus-Ausreißer bei 34% —
#              kein biologisches Signal, Fit nicht interpretierbar.
# P1 Wet B4:   Konfluenz konstant 21-33% ohne Wachstumstrend (rho=0.10) —
#              vermutlich initiales Fokus-Artefakt, kein echtes Wachstum.
EXPLICIT_EXCLUSIONS = {
    ('P1', 'Fibrinogen Coating (Dried) 100 mg/ml (A5)'),
    ('P1', 'Fibrinogen Coating 100 mg/ml (B4)'),
}

# TOST-Äquivalenzgrenzen
TOST_BOUND_A    = 10.0   # Prozentpunkte
TOST_BOUND_MU   = 0.30   # relativ
...
def find_cell_plane(f_values):
    """
    Gibt (position, fraction) des wahrscheinlichsten Zellebenen-Modus zurück.
    Zellebene = höchster Fokus-Modus ≥ CELL_FOCUS_MIN µm.
    Kein Mindestanteil gefordert.
    Gibt (None, 0) wenn kein Modus ≥ CELL_FOCUS_MIN existiert.
    """
...
def gompertz_offset(t, C0, A, mu, lag):
    """
    Gompertz mit Baseline-Offset C0.
    C(t) = C0 + A * exp(-exp((mu*e/A)(lag - t) + 1))
    C0  = initiale Konfluenz bei t=0 (Aussaat-Level)
    A   = zusätzliches Wachstum über C0
    mu  = max. spezifische Wachstumsrate
    lag = Lag-Zeit
    Bei t→-∞: C → C0. Bei t→+∞: C → C0 + A.
    """
    return C0 + A * np.exp(-np.exp((mu * np.e / A) * (lag - t) + 1.0))


def gompertz_offset_decline(t, C0, A, mu, lag, kd, td):
    """Gompertz mit Offset + Post-Peak-Decline."""
...
def logistic4_offset(t, C0, A, k, tmid):
    """4PL mit explizitem Startwert C0."""
...
def _fit(fn, t, y, p0, bounds, k):
    """Fit one model and score it with AICc.

    For least squares with Gaussian errors the number of estimated parameters K
    counts the residual variance as well, hence k + 1 (Burnham & Anderson 2002).
    The small-sample correction is not optional here: the shortest included well
    carries 19 valid timepoints against up to 6 curve parameters, and plain AIC
    would systematically favour the richer model at that length. AICc converges
    to AIC as n/K grows, so the longer wells are unaffected.
    """
    try:
...
def fit_well(t, y):
    """
    Fittet 4 Kandidatenmodelle, alle mit explizitem C0-Offset.
    C0 = Startkonfluenz bei t=0 (erste Messung als Initialwert).
    Dadurch startet die Fit-Kurve bei t=0 beim biologisch sinnvollen
    Aussaat-Level, nicht bei 0%.
    """
    n       = len(t)
    ...
    # ── Linear (Fallback für COC-Flachkurven) ───────────────────────
    ...
    # Modellwahl: sparsamstes Modell innerhalb ΔAICc < 4. Bei gleicher
    # Parameterzahl entscheidet der kleinere AICc-Wert und NICHT die Reihenfolge,
    # in der die Kandidaten eingetragen wurden. Ohne dieses zweite Kriterium
    # haengt die Wahl zwischen zwei gleich sparsamen Modellen an der
    # Dictionary-Reihenfolge, was den Fit-Parametern eine willkuerliche
    # Komponente gibt.
    ...
    # Einheitliche Parameter-Schnittstelle.
    #
    # ACHTUNG, die vier Modelle parametrisieren die Amplitude NICHT gleich:
    # bei Gompertz (mit und ohne Decline) und beim linearen Fit ist der zweite
    # Parameter der ZUWACHS ueber die Startkonfluenz C0, beim 4PL dagegen die
    # Asymptote selbst. Wird beides unbesehen als "A" berichtet, mischt jede
    # Auswertung ueber A zwei verschiedene Groessen, je nachdem welches Modell
    # ein Well zufaellig gewaehlt hat.
    #
    # Berichtet wird daher einheitlich das PLATEAU (obere Asymptote), denn genau
    # das meint die Achsenbeschriftung "Max. Confluence A (%)" und genau das
    # geht in die Statistik ein. Der Zuwachs bleibt als A_gain erhalten.
    p = chosen['popt']
    if name == 'gompertz':
        ...
        # Eine Gerade hat keine Asymptote. Als Plateau wird der Fit-Wert zu
        # einem FESTEN Bezugszeitpunkt T_REF berichtet und nicht am jeweiligen
        # Messende: die Messdauern der Wells unterscheiden sich um bis zu 20 h,
        # was sonst als scheinbare Streuung zwischen den Wells auftaucht.
        C0      = max(float(p[1]), 0.0)
        zuwachs = max(float(p[0]) * T_REF, 0.0)
        mu_max  = max(float(p[0]), 0.0)
        lag, kd, td = 0.0, 0.0, t.max()

    A = C0 + zuwachs          # Plateau, einheitlich ueber alle vier Modelle
...
def assess_well(well, plate_label, conf_df, focus_df):
    ...
    # Expliziter, dokumentierter Ausschluss (siehe EXPLICIT_EXCLUSIONS)
    ...
    # Align on shared Elapsed timestamps (conf and focus may have different row counts)
    merged = pd.merge(
    ...
    # Valide Zeitpunkte im Toleranzband
...
def tost(x, y, bound):
    ...
    The standard error is the unpooled (Welch) one, because the two conditions
    are not assumed to share a variance. The degrees of freedom must then be the
    Welch-Satterthwaite ones as well: using the pooled df = n1 + n2 - 2 together
    with an unpooled SE mixes two different tests and overstates the df whenever
...
def plot_qc(all_wells, fname='fibrinogen_v3_qc.png'):
    ...
    for ax, r in zip(axes, all_wells):
        ...
        # einheitlicher Stil: Box zu, Ticks nur unten/links nach innen, Grid aus
...
# ─────────────────────────────────────────────────────────────
#  PLOT 2: Wachstumskurven — Fit-basiert ab t=0
# ─────────────────────────────────────────────────────────────
def plot_growth(all_wells, fname='fibrinogen_v3_growth.png'):
    ...
    for cond in COND_LIST:
        ...
        # Individuelle Fit-Kurven (dünne Spaghetti) ab t=0
        for r in passed:
        ...
        # Mittlere Fit-Kurve + 95% CI über alle passing Wells
        fit_matrix = np.array([
            np.clip(r['fit']['fn'](t_grid, *r['fit']['popt']), 0, 100)
            for r in passed
        ])                                    # shape: (n_wells, T_GRID_N)
    ...
    # Achsen & Stil — einheitlicher Plot-Stil (Titel 20 | Achsen 16 | Legende 14 | Ticks 12)
    # Box geschlossen: alle 4 Spines sichtbar
    for sp in ax.spines.values():
...
def plot_params(all_wells, fname='fibrinogen_v3_params.png'):
    ...
    # fixed seed: the horizontal jitter is cosmetic, but without a seed the
    # figure differs between runs and could not be reproduced from the data
